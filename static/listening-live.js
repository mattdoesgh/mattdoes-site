...
async function tick() {
  try {
    ...
    // TTL can rewrite that to a long max-age on edge HITs, which would freeze
    // live updates behind a stale browser cache. Force a fresh poll each time;
    // the request path is a cheap KV read (ADR 0008).
