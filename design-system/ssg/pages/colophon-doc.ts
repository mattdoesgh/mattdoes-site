...
<span class="b">├──</span> <span class="d">design-system</span>        <span class="note"># @mattdoes/ds — React page components · renders every page · syncs to Claude Design · ships the timeline island</span>
...
<span class="b">├──</span> <span class="d">workers</span>              <span class="note"># listening · geo · csp-report · lib (shared edge transport)</span>
...
<span class="v">writeFeeds</span>(<span class="s">'dist/feed.xml'</span>)</div>
    ...
    <div class="doc-body">
      <p>One pass, two modules. Intake turns notes into the content model — frontmatter validation, thought splitting, stable IDs, the slug index. Emit writes the model to <code>dist/</code> — markdown, React-rendered pages, hashed assets, feeds. Every public page is a React + TypeScript component in <code>design-system/</code> (<code>@mattdoes/ds</code>), rendered to static HTML at build time — the pages ship without page-wide hydration, and the only client-side React is a small island that enhances the timeline filter/density controls, built as a separate browser bundle and replacing the old hand-written tag filter; the same component library doubles as the design source synced to Claude Design for visual editing. Seven surfaces in the nav (home · writing (/blog/) · thoughts · making · listening · search · about); a new section (say, <code>publish: recipes</code>) is a new page component and one line in <code>routeFor()</code>. Each post also gets a build-time Open Graph card — a 1200×630 PNG rendered with <code>sharp</code> into <code>/og/</code> — plus a related-by-tag block, and a static <code>/search-index.json</code> backs the client-filtered <code>/search/</code>.</p>
      <p>Two dynamic surfaces, both same-origin Workers; <code>connect-src 'self'</code> holds. On <code>/api/listening/*</code> a single-writer Durable Object — the <code>ListeningPoller</code> — is the only thing that calls Last.fm: a self-rescheduling ~25s alarm makes one <code>user.getrecenttracks</code> call, derives both the now-playing pill and the recent-tracks list from it, and writes them to KV; a cron watchdog re-arms the alarm if it ever stops. The request path just reads KV and serves — it never calls Last.fm and never blocks, so upstream volume is constant whatever the traffic. <code>/api/geo/lookup</code> reverse-geocodes a visitor's coords against Nominatim if they opt in via the tweaks panel — by default the animated background renders the home polygon baked into <code>static/home.geojson</code>, no prompt, no network call. Every Worker answers through one shared envelope (<code>workers/lib/transport.js</code>) — JSON + CORS, preflight, cached error responses — with caching policy kept per-Worker.</p>
      <p>Media takes the long way around. <code>scripts/optimize-media.js</code> hashes every attachment and emits <code>.webp</code> siblings into <code>.cache/media-build/</code>; <code>scripts/sync-media.js</code> PUTs originals + variants to R2 over <code>wrangler r2 object</code>, and the build emits <code>&lt;picture&gt;</code> tags pointed at <code>media.mattdoes.online</code>. CSS and JS are content-hashed and served immutable from Pages; the CSP is strict — no third-party connect and no <code>'unsafe-inline'</code> anywhere. The few inline <code>&lt;head&gt;</code> scripts the document shell emits — the importmap and the speculation rules that prerender same-origin links — are admitted by a per-build <code>sha256</code> in <code>script-src</code>, recomputed each build so the hashes track the content-hashed module URLs. <code>style-src</code> stays <code>'self'</code> too (Shiki's per-token colors are de-duplicated into build-time CSS classes rather than inline styles). A Report-Only policy mirrors the enforced one to a same-origin <code>/api/csp-report</code> sink, so the next tightening is measured against real traffic first. Mail is Fastmail, contact is a plain <code>mailto:</code>, no form worker.</p>
    </div>`;
