...
// (ADR 0003). lib/emit.js stays plain Node — it imports string-returning
// render* functions from the built SSG bundle (design-system/dist-ssg/ssg.js);
...
import { resetShikiClasses, classifyShikiHtml, shikiClassCss } from './shiki-csp.js';
 ...
 */
function loadMediaVariants(manifestPath = DEFAULT_MEDIA_MANIFEST) {
...
/**
 * Admit the inline `<head>` importmap under the strict CSP. The importmap is an
 * inline script, and `script-src` carries neither `'unsafe-inline'` nor an
 * importmap analogue of `'inline-speculation-rules'` — so without an explicit
 * hash the browser drops it, every module that imports through it
 * (listening-live.js → `./rows.js`) resolves its clean URL to a 404 HTML page,
 * fails the module MIME check, and live updates die silently. Mirrors
 * emitEarlyHintLinks: post-processes the dist `_headers` copy (never the
 * `static/` source) so each build starts fresh.
 *
 * The importmap's bytes — and therefore the hash — move whenever rows.js or
 * _helpers.js change (their hashed URLs move), so the hash is derived here from
 * the same buildImportmap() the document emits, never hardcoded in static/.
 *
 * @param {string} distDir resolved dist root
 * @param {Record<string, string>} map original → hashed filename
 * @returns {void}
 */
function injectImportmapCsp(distDir, map) {
  const importmap = buildImportmap(map);
  const hash = `'sha256-${crypto.createHash('sha256').update(importmap).digest('base64')}'`;
  const headersPath = path.join(distDir, '_headers');
  let txt = fs.readFileSync(headersPath, 'utf8');
  // Append the hash to every script-src directive (the enforced CSP and its
  // report-only twin), right after the inline-speculation-rules source.
  // Idempotent: leave a directive untouched if it already carries this hash.
  txt = txt.replace(/script-src [^;]*?'inline-speculation-rules'/g, (directive) =>
    directive.includes(hash) ? directive : `${directive} ${hash}`);
  // Fail loud if the anchor didn't match. Shipping a CSP that still drops the
  // importmap is the exact silent regression this mechanism exists to prevent,
  // and `npm run build` (the deploy path) skips the test that would otherwise
  // catch it — so anchor drift (a future script-src edit) must surface here, at
  // build time, not as dead live-updates in production.
  const enforced = txt.split('\n').find(l => l.trim().startsWith('Content-Security-Policy:'));
  if (!enforced || !enforced.includes(hash)) {
    throw new Error(
      `injectImportmapCsp: ${headersPath} script-src did not receive the importmap `
      + `hash ${hash} — the strict CSP would drop the inline importmap. `
...
export async function emit(model, {
  ...
  mediaVariants = loadMediaVariants(mediaManifest || DEFAULT_MEDIA_MANIFEST);
  altWarned.clear();
  resetShikiClasses();
  ...
  injectImportmapCsp(DIST_DIR, assetMap);
  ...
  // Homepage. Listening rows are dropped from the timeline — scrobbles live on
  // the dedicated /listening/ page; the homepage still surfaces the scrobble
  // count and now-playing status via siteMeta. Mirrors the blog page below,
  // which also excludes listening.
  fs.writeFileSync(path.join(DIST_DIR, 'index.html'), renderIndex({ site: siteMeta, entries: feedEntries.filter(e => e.kind !== 'listening'), siteConfig, assets: assetMap }));
