 ...
 * the hashed rows.js/_helpers.js URLs, so hashes are derived here from the same
 * build*() helpers the document emits, never hardcoded in static/.
 *
 * @param {string} distDir resolved dist root
 * @param {Record<string, string>} map original → hashed filename
 * @returns {void}
 */
function injectInlineScriptCsp(distDir, map) {
  // Every inline <script> the document shell emits (document.tsx). Keep this in
  // lockstep with what renderDocument inlines — a new inline script added there
  // without a hash here is silently blocked by the strict CSP.
  const inlineScripts = [buildImportmap(map), buildSpeculationRules()];
...
export async function emit(model, {
  ...
  injectInlineScriptCsp(DIST_DIR, assetMap);
