...
/** `user.getinfo` request URL (total playcount lives here). */
export function userInfoUrl(user, key) {
  return `${API_ROOT}?method=user.getinfo&user=${encodeURIComponent(user)}&api_key=${encodeURIComponent(key)}&format=json`;
}
 ...
 * and reads the total playcount from the response's `@attr.total` (also
 * persisted in the build-time tracks cache; user.getinfo remains the
 * preferred source when available — see {@link decodePlaycount}).
 *
 * @param {object} body  parsed JSON response
 * @param {object} [opts]
...
/**
 * Total scrobble count from a `user.getinfo` response body.
 *
 * @param {object} body  parsed JSON response
 * @returns {number}
 */
export function decodePlaycount(body) {
  return Number(body?.user?.playcount) || 0;
}
