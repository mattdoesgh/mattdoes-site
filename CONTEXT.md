...
Each Row exists in two forms, with a deliberate source-of-truth split. The string renderers in `templates/rows.js` (`articleRow`/`thoughtRow`/`listeningRow`/`emptyState`) are the canonical markup. The React mirrors in `design-system/src/components/` (`ArticleRow`/`ThoughtRow`/`ListeningRow`/`EmptyState`) are the `@mattdoes/ds` public API synced to Claude Design. Articles and thoughts actually render the site through their React components (semantic-equivalence proven by `design-system/ssg/render.tsx`), so they can't silently drift. **Listening is the exception**: `ListeningRow.tsx` is *only* the Claude Design mirror — the shipped /listening/ page injects `templates/rows.js` output as `rowsHtml` (byte-equality with the browser live-update module, ADR 0001 + `test/row-parity.test.js`), so React never renders it and nothing mechanically catches drift. Treat `listeningRow()` as canonical and mirror changes into `ListeningRow.tsx` by hand; the file header records this.
